...
pharmacists_data_raw = readRDS("Pharmacists_2022_v5.0.rds")
...
pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(stores_working_at = ifelse(workplaces == "This pharmacy professional has not reported a workplace.", 0, stores_working_at)) %>%
    mutate(stores_working_at = ifelse(workplaces == "This person is not entitled to practice and does not have a workplace.", 0, stores_working_at))
...
# Getting suspenion times

pharmacists_data_cleaned = pharmacists_data_cleaned %>%  
    mutate(reg_history_txt_2 = reg_history_txt) %>% 
    select(reg_history_txt_2, everything())

pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(license_suspended_for_discipline_times = ifelse(str_detect(reg_history_txt_2, "Suspended for Discipline"), 1, 0)) %>% 
    mutate(reg_history_txt_2 = str_after_nth(reg_history_txt_2, "Suspended for Discipline", n = 1)) %>% 
    mutate(reg_history_txt_2 = str_after_nth(reg_history_txt_2, "\\|", n = 1))

pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(suspension_start_first_from_now = str_before_nth(reg_history_txt_2, "\\|", n = 1) %>% mdy()) %>% 
    mutate(reg_history_txt_2 = str_after_nth(reg_history_txt_2, "\\|", n = 1))

pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(suspension_end_first_from_now = str_before_nth(reg_history_txt_2, "\\|", n = 1) %>% mdy()) %>% 
    mutate(reg_history_txt_2 = str_after_nth(reg_history_txt_2, "\\|", n = 1))

pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(total_time_suspended_days = ifelse(!is.na(suspension_start_first_from_now) & !is.na(suspension_end_first_from_now),
                                              (suspension_end_first_from_now - suspension_start_first_from_now) %>% as.character() %>% str_remove("days") %>% as.numeric(),
                                              0))
#
...
pharmacists_data_cleaned = pharmacists_data_cleaned %>%
    mutate(license_suspended_for_discipline_times = ifelse(!is.na(reg_history_txt_2), license_suspended_for_discipline_times + 1, license_suspended_for_discipline_times))


pharmacists_data_cleaned = pharmacists_data_cleaned %>%
    mutate(suspension_start_second_time_from_now = str_before_nth(reg_history_txt_2, "\\|", n = 1) %>% mdy()) %>%
    mutate(reg_history_txt_2 = str_after_nth(reg_history_txt_2, "\\|", n = 1))

pharmacists_data_cleaned = pharmacists_data_cleaned %>%
    mutate(suspension_end_second_time_from_now = str_before_nth(reg_history_txt_2, "\\|", n = 1) %>% mdy()) %>%
    mutate(reg_history_txt_2 = str_after_nth(reg_history_txt_2, "\\|", n = 1))

pharmacists_data_cleaned = pharmacists_data_cleaned %>%
    mutate(total_time_suspended_days = ifelse(!(is.na(suspension_start_second_time_from_now) & is.na(suspension_end_second_time_from_now)),
                                              total_time_suspended_days + (suspension_end_second_time_from_now - suspension_start_second_time_from_now) %>% as.character() %>% str_remove("days") %>% as.numeric(),
                                              total_time_suspended_days))
#
...
pharmacists_data_cleaned = pharmacists_data_cleaned %>%
    mutate(suspension_start_third_time_from_now = str_before_nth(reg_history_txt_2, "\\|", n = 1) %>% mdy()) %>%
    mutate(reg_history_txt_2 = str_after_nth(reg_history_txt_2, "\\|", n = 1))

pharmacists_data_cleaned = pharmacists_data_cleaned %>%
    mutate(suspension_end_third_time_from_now = str_before_nth(reg_history_txt_2, "\\|", n = 1) %>% mdy()) %>%
    mutate(reg_history_txt_2 = str_after_nth(reg_history_txt_2, "\\|", n = 1))

pharmacists_data_cleaned = pharmacists_data_cleaned %>%
    mutate(total_time_suspended_days = ifelse(!(is.na(suspension_start_third_time_from_now) & is.na(suspension_end_third_time_from_now)),
                                              total_time_suspended_days + (suspension_end_third_time_from_now - suspension_start_third_time_from_now) %>% as.character() %>% str_remove("days") %>% as.numeric(),
                                              total_time_suspended_days))



pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    select(-reg_history_txt_2) 

pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(reg_history_txt_2 = reg_history_txt)

pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(reg_history_txt = ifelse(str_detect(reg_history_txt, "\\|Back to search"),
                                                                yes = str_before_nth(reg_history_txt, "Back to search", n = -1),
                                                                no = reg_history_txt)) %>% 
    mutate(reg_history_txt = ifelse(str_detect(reg_history_txt, "Registration in Other Jurisdictions\\|"),
                                    yes = str_before_nth(reg_history_txt, "Registration in Other Jurisdictions\\|", n = 1),
                                    no = reg_history_txt))
    for (i in 1:100) {
    
pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(reg_history_txt = ifelse(str_detect(reg_history_txt, "\\|Pharmacist\\|"),
                                    yes = str_after_nth(reg_history_txt, "\\|Pharmacist\\|", n = 1),
                                    no = reg_history_txt))
}

pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(initial_pharmacist_registration = ifelse(str_before_nth(reg_history_txt, "\\|", n = 1) == "Current" & role == "Pharmacist",
                                    yes = str_after_nth(reg_history_txt, "\\|", n = 1),
                                    no = "")) %>% 
    mutate(initial_pharmacist_registration = ifelse(initial_pharmacist_registration != "",
                                                    yes = str_before_nth(initial_pharmacist_registration, "\\|", n = 1),
                                                    no = initial_pharmacist_registration)) %>% 
    mutate(reg_history_txt = ifelse(str_before_nth(reg_history_txt, "\\|", n = 1) == "Current" & role == "Pharmacist",
                                                    yes = "",
                                                    no = reg_history_txt)) %>% 
    mutate(initial_pharmacist_registration = ifelse(str_before_nth(reg_history_txt, "\\|", n = 1) != "\\|Status" & role == "Pharmacist" & initial_pharmacist_registration == "",
                                                    yes = str_before_nth(reg_history_txt, "\\|", n = 1),
                                                    no = initial_pharmacist_registration)) %>% 
    mutate(reg_history_txt = ifelse(str_before_nth(reg_history_txt, "\\|", n = 1) != "\\|Status" & role == "Pharmacist",
                                                    yes = "",
                                                    no = reg_history_txt)) %>%
    mutate(initial_pharmacist_registration = ifelse(str_before_nth(reg_history_txt, "\\|", n = 1) == "Resigned" & role == "Pharmacist" & initial_pharmacist_registration == "Resigned",
                                                    yes = str_before_nth(reg_history_txt, "\\|", n = 2) %>% str_after_nth("\\|", n = 1),
                                                    no = initial_pharmacist_registration)) %>% 
    mutate(initial_pharmacist_registration = ifelse(initial_pharmacist_registration == "" & role == "Pharmacist Emeritus" & str_before_nth(reg_history_txt, "\\|", n = 1) == "Current",
                                                    yes = str_before_nth(reg_history_txt, "\\|", n = 2) %>% str_after_nth("\\|", n = 1),
                                                    no = initial_pharmacist_registration)) %>% 
    mutate(reg_history_txt = ifelse(role == "Pharmacist Emeritus" & str_before_nth(reg_history_txt, "\\|", n = 1) == "Current",
                                    "", reg_history_txt)) %>% 
    mutate(initial_pharmacist_registration = ifelse(initial_pharmacist_registration == "" & role == "Pharmacist Emeritus" & str_before_nth(reg_history_txt, "\\|", n = 1) != "Current",
                                                    yes = str_before_nth(reg_history_txt, "\\|", n = 1),
                                                    no = initial_pharmacist_registration)) %>% 
...
pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(initial_pharmacist_registration = initial_pharmacist_registration %>% mdy() )

pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(reg_history_txt = reg_history_txt_2) %>% 
    select(-reg_history_txt_2)
...
pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(pharmacist_current_initial_registration_date = ifelse(str_detect(reg_history_txt, "Current\\|") & role == "Pharmacist",
                                                                 yes = str_after_nth(reg_history_txt, "Current\\|", n = 1),
                                                                 no = "")) %>% 
    mutate(reg_history_txt = ifelse(str_detect(reg_history_txt, "Current\\|")  & role == "Pharmacist",
                                    yes = "",
                                    no = reg_history_txt)) %>% 
...
pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(pharmacist_current_initial_resignation_date = ifelse(str_detect(reg_history_txt, "Resigned\\|") & role == "Pharmacist",
                                                                yes = str_after_nth(reg_history_txt, "Resigned\\|", n = 1),
                                                                no = "")) %>% 
    mutate(reg_history_txt = ifelse(str_detect(reg_history_txt, "Resigned\\|") & role == "Pharmacist",
                                    yes = "",
                                    no = reg_history_txt)) %>% 
    mutate(pharmacist_current_initial_resignation_date = pharmacist_current_initial_resignation_date  %>% mdy())

pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(pharmacist_current_cancelled_or_suspended_for_nonpayment_date = ifelse(str_detect(reg_history_txt, "Cancelled for Non-payment\\|") & role == "Pharmacist",
                                                                                  yes = str_after_nth(reg_history_txt, "Cancelled for Non-payment\\|", n = 1),
                                                                                  no = "")) %>% 
    mutate(reg_history_txt = ifelse(str_detect(reg_history_txt, "Cancelled for Non-payment\\|")& role == "Pharmacist",
                                    yes = "",
                                    no = reg_history_txt)) %>% 
    mutate(pharmacist_current_cancelled_or_suspended_for_nonpayment_date = ifelse(str_detect(reg_history_txt, "Suspended for Non-payment\\|")  & role == "Pharmacist",
                                                                                  yes = str_after_nth(reg_history_txt, "Suspended for Non-payment\\|", n = 1),
                                                                                  no = pharmacist_current_cancelled_or_suspended_for_nonpayment_date)) %>% 
    mutate(reg_history_txt = ifelse(str_detect(reg_history_txt, "Suspended for Non-payment\\|") & role == "Pharmacist",
                                    yes = "",
                                    no = reg_history_txt)) %>% 
...
pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(pt_current_cancelled_or_suspended_for_nonpayment_date = ifelse(str_detect(reg_history_txt, "Cancelled for Non-payment\\|")  & role == "Pharmacy Technician",
                                                                          yes = str_after_nth(reg_history_txt, "Cancelled for Non-payment\\|", n = 1),
                                                                          no = "")) %>% 
    mutate(reg_history_txt = ifelse(str_detect(reg_history_txt, "Cancelled for Non-payment\\|") & role == "Pharmacy Technician",
                                    yes = "",
                                    no = reg_history_txt)) %>% 
    mutate(pt_current_cancelled_or_suspended_for_nonpayment_date = ifelse(str_detect(reg_history_txt, "Suspended for Non-payment\\|" ) & role == "Pharmacy Technician",
                                                                          yes = str_after_nth(reg_history_txt, "Suspended for Non - Payment\\|", n = 1),
                                                                          no = pt_current_cancelled_or_suspended_for_nonpayment_date)) %>% 
    mutate(reg_history_txt = ifelse(str_detect(reg_history_txt, "Suspended for Non-payment\\|") & role == "Pharmacy Technician",
                                    yes = "",
                                    no = reg_history_txt)) %>% 
    mutate(pt_current_cancelled_or_suspended_for_nonpayment_date = pt_current_cancelled_or_suspended_for_nonpayment_date  %>% mdy())

pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(intern_current_initial_registration_date = ifelse(str_detect(reg_history_txt, "Current\\|") & role == "Intern",
                                                             yes = str_after_nth(reg_history_txt, "Current\\|", n = 1),
                                                             no = "")) %>% 
    mutate(reg_history_txt = ifelse(str_detect(reg_history_txt, "Current\\|") & role == "Intern",
                                    yes = "",
                                    no = reg_history_txt)) %>% 
    mutate(intern_current_initial_registration_date = intern_current_initial_registration_date  %>% mdy())

pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(student_current_initial_registration_date = ifelse(str_detect(reg_history_txt, "Current\\|") & role == "Student",
                                                              yes = str_after_nth(reg_history_txt, "Student\\|Current\\|", n = 1),
                                                              no = "")) %>% 
    mutate(reg_history_txt = ifelse(str_detect(reg_history_txt, "Current\\|") & role == "Student",
                                    yes = "",
                                    no = reg_history_txt)) %>% 
    mutate(student_current_initial_registration_date = student_current_initial_registration_date  %>% mdy())

pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(student_current_initial_expired_date = ifelse(str_detect(reg_history_txt, "Expired\\|") & role == "Student",
                                                         yes = str_after_nth(reg_history_txt, "Student\\|Expired\\|", n = 1),
                                                         no = "")) %>% 
    mutate(reg_history_txt = ifelse(str_detect(reg_history_txt, "Expired\\|") & role == "Student",
                                    yes = "",
                                    no = reg_history_txt)) %>% 
    mutate(student_current_initial_expired_date = student_current_initial_expired_date  %>% mdy())

pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(pt_current_initial_reg_date = ifelse(str_detect(reg_history_txt, "Current\\|") & role == "Pharmacy Technician",
                                                yes = str_after_nth(reg_history_txt, "Pharmacy Technician\\|Current\\|", n = 1),
                                                no = "")) %>% 
    mutate(reg_history_txt = ifelse(str_detect(reg_history_txt, "Current\\|") & role == "Pharmacy Technician",
                                    yes = "",
                                    no = reg_history_txt)) %>% 
    mutate(pt_current_initial_reg_date = pt_current_initial_reg_date  %>% mdy())

pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(pt_current_initial_resigned_date = ifelse(str_detect(reg_history_txt, "Resigned\\|") & role == "Pharmacy Technician",
                                                     yes = str_after_nth(reg_history_txt, "Pharmacy Technician\\|Resigned\\|", n = 1),
                                                     no = "")) %>% 
    mutate(reg_history_txt = ifelse(str_detect(reg_history_txt, "Resigned\\|") & role == "Pharmacy Technician",
                                    yes = "",
                                    no = reg_history_txt)) %>% 
...
pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(pharmacist_revoked_date = ifelse(str_detect(reg_history_txt, "Revoked\\|") & role == "Pharmacist",
                                                                 yes = str_after_nth(reg_history_txt, "Revoked\\|", n = 1),
                                                                 no = "")) %>% 
    mutate(reg_history_txt = ifelse(str_detect(reg_history_txt, "Revoked\\|") & role == "Pharmacist",
                                    yes = "",
                                    no = reg_history_txt)) %>% 
    mutate(pharmacist_revoked_date = pharmacist_revoked_date  %>% mdy())
...
pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(education_graduation_date = "") %>% 
    mutate(education_graduation_date = ifelse(str_detect(education_country, "Graduated: "),
                                      str_after_nth(education_country, "Graduated: ", n = 1),
...
pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(concerns_txt = ifelse(str_detect(concerns_txt, "\\|Discipline\\|"),
                                 str_before_nth(concerns_txt, "\\|Discipline\\|", n = 1), concerns_txt))

pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(concerns_proffesional_misconduct_times = str_count(concerns_txt, "Professional Misconduct")) %>% 
    mutate(concerns_guilt_times = str_count(concerns_txt, "Federal or Provincial Findings of Guilt")) %>% 
    mutate(concerns_propriety_misconduct_times = str_count(concerns_txt, "Proprietary Misconduct")) %>% 
    mutate(concerns_caution_training_times = str_count(concerns_txt, "Caution and Remedial Training")) %>% 
    mutate(concerns_federal_charges_times = str_count(concerns_txt, "Federal or Provincial Charges")) %>% 
    mutate(concerns_application_reinstatement_times = str_count(concerns_txt, "Application for Reinstatement")) %>% 
    mutate(concern_outcome_caution_times = str_count(concerns_txt, "Caution from OCP")) %>% 
    mutate(concerns_pending_trial_times = str_count(concerns_txt, "Pending trial"))
...
pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    select(name, role, status, registration_number,
           ...
           international_grad, concerns, license_suspended_for_discipline_times, total_time_suspended_days, everything())

pharmacists_data_cleaned = pharmacists_data_cleaned %>% distinct(registration_number, .keep_all = TRUE)

pharmacists_data_cleaned = pharmacists_data_cleaned %>% 
    mutate(pharmacy_locations_primary_keys_url = ifelse(str_starts(pharmacy_locations_primary_keys_url, "\\,"), str_remove(pharmacy_locations_primary_keys_url, "\\,"), pharmacy_locations_primary_keys_url))

write_rds(pharmacists_data_cleaned, "OCP_Pharmacists_2022_V12.rds")
